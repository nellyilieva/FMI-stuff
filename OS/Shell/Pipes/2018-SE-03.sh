...
# cat /etc/passwd | sort -t ':' -k3 -n | head -n 201 | grep '/home/SI/' | cut -d ':' -f 5,6 | sed 's/,.*//' | sort -t '/' -k3 -> NO

cat /etc/passwd | sort -t ':' -k3 -n | head -n 201 | tail -n 1 | awk -F ':' '{print $4}' | xargs -I{} grep -F "{}" /etc/passwd | awk -F ':' '{print $5 " " $6}' | tr ',' ' ' | awk '{print $1 " " $2 ":" $NF}' | sort -t 's' -k2 -n
